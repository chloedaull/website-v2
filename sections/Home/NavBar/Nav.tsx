...
const Nav = ({ openNav }: Props) => {
  ...
  const [showContact, setShowContact] = useState(false); // <-- modal toggle state
        ...
        <div className="flex items-center h-full justify-between w-[90%] xl:w-[80%] mx-auto">
          ...
          <div className="flex items-center space-x-2">
            <div className="w-10 h-10 bg-white-800 rounded-full flex items-center justify-center flex-col">
              <Si4Chan className="w-6 h-6 text-white" />
            </div>
          ...
          {/* NavLinks */}
          <div className="hidden lg:flex items-center space-x-10 pl-80">
            {navLinks.map((link) => (
              <button
                key={link.id}
                onClick={(e) => {
          ...
          <div className="flex items-center space-x-4">
            <button
              onClick={() => setShowContact(true)} // <-- show modal on click
